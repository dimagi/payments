...
from hqpayments.utils import get_mach_data, format_start_end_suffixes
...
class HQMonthlyBill(Document):
    """
        This bill is auto-generated by Celery for the previous month starting at midnight on the first of each month.
    """
    ...
    def base_couch_view(cls):
        return "hqpayments/monthly_bills"

    @classmethod
    def get_bills(cls, domain, prefix="start", start=None, end=None, include_docs=True):
        key = [prefix, domain]
    ...
    def create_bill_for_domain(cls, domain, billing_range=None):
        ...
        all_incoming = SMSBillableItem.by_domain_and_direction(domain, INCOMING,
        ...
        all_outgoing = SMSBillableItem.by_domain_and_direction(domain, OUTGOING,
                            start=start.isoformat(), end=end.isoformat())
        ...
        active_users = bill.get_all_active_and_submitted_users()
    ...
    def get_all_active_and_submitted_users(self):
        """
            Getting the unique set of all the currently active and all
            registered users who have submitted over the past month.
        """
    ...
    def get_ids_and_cost(billables):
        return [b.get_id for b in billables], sum([b.conversion_rate*b.billable_amount for b in billables])

    @staticmethod
    def get_default_start_end():
        today = datetime.datetime.utcnow()+datetime.timedelta(days=30)
...
class SMSBillableItem(Document):
    """
        One of these is generated every time an SMS is successfully sent and a matching SMSBillableRate can be found
        for the API and method used.
    """
    billable_date = DateTimeProperty()
    billable_amount = DecimalProperty() # billable amount is not converted into USD, stays in the billing rate's currency
